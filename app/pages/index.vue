...
const socials: { platform: string; url: string; username: string }[] = [
  ...
  {
    platform: "git",
    url: "https://git.luvsick.gg/luvsick",
    username: "@luvsick",
  },
  {
    platform: "steam",
    ...
    <main class="w-full max-w-[920px] grid grid-cols-1 gap-5">
      ...
      >
        ...
        <p class="m-0 mb-[14px] text-white/60 text-[13px]">
          ...
          developer. lately i've been diving into digital privacy, open source,
          security, and transparency. i'm also into playing with infrastructure
          and self-hosting, as well as playing with modern tech.
        </p>
        <p class="mt-3 mb-0 text-white/60 text-[13px] leading-6">
          you'll probably also find me playing some games, listening to music,
          messing with my dotfiles, or just existing on the internet.
        </p>
      </section>
          ...
          >
            ...
            >
              <div class="min-w-0 flex-1 flex flex-col gap-[2px]">
                <span class="font-semibold text-sm">{{ social.platform }}</span>
                <em
      ...
      >
        ...
        <div class="mt-2">
          <div class="text-white/60 flex gap-2 items-center mb-3 text-[13px]">
            <span>currently, i operate</span>
            <strong class="text-white">{{ servers.length }}</strong>
            <span>{{ servers.length === 1 ? "server" : "servers" }}.</span>
          </div>
        ...
        <div class="mt-4" v-if="domains.length">
          <div class="text-white/60 flex gap-2 items-center mb-3 text-[13px]">
            ...
            <span
              >{{ domains.length === 1 ? "root domain" : "root domains" }} at
              the registry level.</span
            >
          </div>
      ...
      <section
        class="p-4 bg-white/[0.03] rounded-xl border border-white/[0.02] text-sm"
        v-if="orgs.length"
      >
        <h3 class="m-0 mb-2">organisations i support</h3>
        <p class="m-0 mb-[14px] text-white/60 text-[13px]">
          i'm proud to be part of and support the following organisations:
        </p>
        <ul class="list-none p-0 mt-2 mb-0 flex flex-col gap-2">
          <li
            v-for="org in orgs"
            :key="org"
            class="py-2 px-[10px] rounded-lg bg-white/[0.01] border border-white/[0.02] text-white/60 text-sm"
          >
            {{ org }}
          </li>
        </ul>
